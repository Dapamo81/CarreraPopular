...
exports.postRegister = async (req, res) => {
    ...
    try {
        const [rows, fields] = await req.pool
            .promise()
            .query("SELECT * FROM corredores WHERE nombre = ?", [username]);
        if (rows.length > 0) {
        ...
        await req.pool
            .promise()
            .query(
                "INSERT INTO corredores (dni, nombre, apellidos, telefono, direccion, poblacion, cp, email, contrasena) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                [
                    dni,
                    username,
                    apellidos,
